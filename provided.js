...
function chooseRandomAnswerTest() {
  ...
  if (_mockRandomNumberCalls.length < 3) {
    ...
    console.log(constructHintMessage(`Invoke chooseRandomAnswer`, `  Start by calling chooseRandomAnswer and assign the result to variable.  Make sure it's min and max value will return a number in the correct range. If you need help with calling/invoking a function, please refer to the Resource Guide at the bottom of this hint.`, RESOURCES.callingFunctions))
    return false;
  }
  ...
  if (positiveAnswer == undefined || negativeAnswer == undefined || maybeAnswer == undefined) {
    ...
    console.log(constructHintMessage(`Return correct array`, ` No value was returned from 'chooseRandomAnswer'. Use an${COLOR_CODE.bright} if/else statement ${COLOR_CODE.reset}${COLOR_CODE.cyan}to compare the value of "answerType" parameter. Remember to use your random number variable to index an answer from the corresponding answer array and return it.`, RESOURCES.conditionals))
    return false;
  }
...
function chooseRandomAnswerTypeTest() {
  ...
  if (_mockRandomNumberCalls.length < 1) {
    ...
    console.log(constructHintMessage(`Invoke chooseRandomAnswer`, `  Call chooseRandomAnswer and assign it to variable. Make sure it's min and max value will return a number in the correct range you need. If you need help with calling/invoking a function, please refer to the Resource Guide at the bottom of this hint.`, RESOURCES.callingFunctions))
  ...
  if (answerTypeSet.hasOwnProperty(undefined)) {
    ...
    console.log(constructErrorMessage(TEST_NAME, `No value was returned from 'chooseRandomAnswerType'.`, ""))
    console.log(constructHintMessage(`No value was returned from 'chooseRandomAnswerType'.`, `  Remember to return an answer type string that is either "positive", "negative" or "maybe". You can use your random number to decide which string you want to return. Example: ${COLOR_CODE.bright}if${COLOR_CODE.reset}${COLOR_CODE.cyan} random number is 0, return "positive". ${COLOR_CODE.bright}else if${COLOR_CODE.reset}${COLOR_CODE.cyan} random number is 1, return "negative". ${COLOR_CODE.bright}Else${COLOR_CODE.reset}${COLOR_CODE.cyan} return "maybe"`, RESOURCES.conditionals));
    return false;
  }
  ...
  if (!answerTypeSet.hasOwnProperty("positive")) {
    ...
    console.log(constructHintMessage(`We called 'chooseRandomAnswerType' 50 times and never got "positive"`, ` Make sure your ${COLOR_CODE.bright} if/else ${COLOR_CODE.reset} ${COLOR_CODE.cyan}returns "positive" at some point.`, RESOURCES.conditionals))
    return false;
  }

  if (!answerTypeSet.hasOwnProperty("negative")) {
    console.log(constructErrorMessage(TEST_NAME, `Did not return "negative".`, ""));
    console.log(constructHintMessage(`We called 'chooseRandomAnswerType' 50 times and never got "negative".`, ` Make sure your ${COLOR_CODE.bright}if/else ${COLOR_CODE.reset} ${COLOR_CODE.cyan}returns "negative" at some point.`, RESOURCES.conditionals))
    return false;
    return false;
  }
  ...
  if (!answerTypeSet.hasOwnProperty("maybe")) {
    ...
    console.log(constructHintMessage(`We called 'chooseRandomAnswerType' 50 times and never got "maybe".`, ` Make sure your ${COLOR_CODE.bright}if/else ${COLOR_CODE.reset} ${COLOR_CODE.cyan}returns "maybe" at some point.`, RESOURCES.conditionals))
    return false;
    return false;
  }
...
function _onAnswerRequestedTest_BlankQuestion() {
  ...
  if (answerText != NO_QUESTION_WARNING) {
    console.log(constructErrorMessage(TEST_NAME, `Logic for no user question. When no question is supplied, 'onAnswerRequested' should call the provided function 'supplyAnswer' with the argument "${NO_QUESTION_WARNING}" (NO_QUESTION_WARNING) but it returned "${answerText}" instead.`, ""));
    console.log(constructHintMessage(`Logic for no user question`, `Make sure that ${COLOR_CODE.bright}if ${COLOR_CODE.reset}${COLOR_CODE.cyan}the user does not supply a question, the answer variable is assigned the "NO_QUESTION_WARNING". ${COLOR_CODE.bright}If${COLOR_CODE.reset}${COLOR_CODE.cyan} the user does not supply a question, the return value of getQuestionText() will be an empty string. `, RESOURCES.conditionals))
    return false;
  }
...
function _onAnswerRequestedTest_ValidQuestion() {
  ...
  if (_mockSupplyAnswerCalls.length == 0) {
    console.log(constructErrorMessage(TEST_NAME, `The 'onAnswerRequested' function never calls 'supplyAnswer'. You need to call 'supplyAnswer' to display the answer to the user.`, ""));
    console.log(constructHintMessage(`The 'onAnswerRequested' function never calls 'supplyAnswer'.`, `Try and make sure your function has 'supplyAnswer' called everytime the function is run. Use your 'answer' variable as the argument. The function should look like this when you call it: example(argument)`, RESOURCES.callingFunctions))
    return false;
  }
  ...
  let answerText = _mockSupplyAnswerCalls[0].answerText;
  if (answerText == NO_QUESTION_WARNING) {
    console.log(constructErrorMessage(TEST_NAME, `User Input Logic. When a question was provided, the 'onAnswerRequested' function returned "${NO_QUESTION_WARNING}". This is only supposed to happen when the user does not provide a question.`, ""));
    console.log(constructHintMessage('User Input Logic', `Try and find the provided function that will return the user's input as a value. Assign the return value of that function to a variable. Then you can use the user's input as a standard JavaScript variable!`))
    return false;
  }
